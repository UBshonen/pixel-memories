...
**갱신일** 2026-08-21 · **브랜치** main

---

## 한 줄 요약

**배포만 남았다.** 코드 준비는 끝났고 Cloudflare Pages 계정 연결부터 하면 된다.
절차는 [DEPLOY.md](DEPLOY.md)에 혼자 따라 할 수 있게 적어뒀다.
...
**[DEPLOY.md](DEPLOY.md) 를 위에서부터 따라 하면 된다.**

```text
[x] 정적 내보내기 설정 (output: "export")
[x] OG 라우트 force-static
[x] public/_headers (OG 이미지 MIME 타입 + 캐시)
[x] README 정비 · 안 쓰는 에셋 제거
[x] 로컬 빌드 통과
[ ] Cloudflare 가입 → 저장소 연결 → 배포     ← 여기부터
[ ] 폰에서 확인 (복사·공유가 https 에서 처음 제대로 동작한다)
[ ] 카카오톡 미리보기 확인
[ ] README · INTERVIEW 에 배포 주소 채우기
```

**Vercel 은 접었다.** 구글로 가입한 계정을 지운 뒤 재가입이 막혔다.
삭제해도 연결이 남는 알려진 문제이고 지원팀 문의는 2~3일 걸린다.
경위와 대안 비교는 [INTERVIEW.md](INTERVIEW.md) §11-8.

---

## 배포 후 — 단계 2 · 그래픽

**내일 첫 할 일은 코드가 아니라 결정이다.**

```text
[ ] 그림을 직접 그릴지, 무료 에셋(Kenney·itch.io)을 쓸지
```

이것이 정해지지 않으면 그림 작업이 시작되지 않는다.
코드 쪽 준비(4방향 구조)는 이미 끝나 있다.
...
**4방향 코드는 이미 끝났다.** 캐릭터가 방향을 기억하고 애니메이션을
갈아끼운다. 위로 걸으면 뒤통수가 보인다. 임시 그림으로 먼저 만들어 두었으므로
진짜 그림이 오면 `placeholderTextures.ts` 의 drawPerson 만 이미지 로딩으로
바꾸면 된다. 프레임 순서는 personFrame() 이 정한다.
...
## 폰에서 확인 완료 (2026-08-19)

앞·뒤·옆 방향 전환, 말풍선 글자 가독성, 3배 확대, 탭 이동 모두 확인했다.

## 알려진 문제

### 손가락 안내가 귀엽지도 않고 눈에 띄지도 않는다

10x12 임시 그림이라 그렇다. 그림 단계(단계 2)에서 함께 그린다.
눈에 안 띄는 것은 크기·색·움직임 문제이므로, 그때 다음을 함께 조정한다.

```text
크기      지금 10x12. 더 크게
색        살구색이라 잔디 위에서 묻힌다. 테두리를 넣거나 대비를 높인다
움직임    한 번 톡 누르고 사라진다. 두세 번 반복하거나 물결을 함께 띄운다
```

## 아직 판단하지 않은 것

- **표지판을 남길지.** 살아 움직이는 오브젝트 · 주민 인사 · 손가락 안내 ·
  고양이가 모두 들어간 지금, 표지판이 여전히 필요한지 확인이 필요하다.
  뺀다면 `villageObjects.ts`의 `obj-signpost-plaza` 한 항목만 지우면 된다.
- **입장 직후 연출이 몰리지 않는지.** 손가락 → 자동 이동 → 고양이 안내가
  연달아 일어난다. 정신없으면 `HINT_DELAY` 와 `IDLE_BEFORE_FIRST_GUIDE` 로 벌린다.
- **오브젝트 흔들림이 과하지 않은지.** `IDLE_BOB_PIXELS` 로 조절.
