...
```text
1. 조작 방식 결정      탭 이동만 vs 방향 버튼 병행     ← 폰으로 비교 중
2. 배율 확정           CAMERA_ZOOM 2가 적당한지 3인지
3. 캐릭터 방향         지금은 정면 그림 하나로 좌우만 뒤집는다.
                       위로 걸어도 얼굴이 보인다.
                       4방향 스프라이트를 만들지 결정 (그림 양 4배)
4. 화면 비율 대응      기기별 세로 길이 차이
5. 시작 화면           "입장하기" (BLUEPRINT §6)
```

**2번과 3번은 그림 작업 전에 반드시 확정해야 한다.** 타일 크기와 방향 수가
바뀌면 이미 그린 그림을 전부 다시 그려야 한다. 그 외에는 뒤집힐 일이 없다.
