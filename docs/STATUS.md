...
지금은 9px로 구운 그림을 카메라가 3배로 늘리는데, pixelArt 모드라
부드럽게 늘리지 않고 픽셀을 그대로 키운다. 한글은 9px에 담을 획이 없어
더 심하게 뭉갠다.

**해법** — Text를 만들 때 `resolution: CAMERA_ZOOM` 을 준다.
글자 그림을 처음부터 3배 크기로 구우면 카메라가 3배로 늘려도 1:1이 되어 또렷해진다.
`▼` 표시에도 같은 문제가 있다.

더 확실한 방법은 말풍선을 Phaser가 아니라 HTML로 그리는 것이다.
글자 UI는 원래 React 담당이라는 역할 분담과도 맞는다. 다만 매 프레임
주민의 화면 좌표를 React에 넘겨야 해서 손이 더 간다.
